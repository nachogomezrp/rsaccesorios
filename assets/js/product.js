/**
 * @file product.js
 * Lógica de la vista de detalle de producto.
 */
...
// ─── Buscador y Navegación en vista Producto ─────────────────────
...
function buildCategoryNav(products) {
  ...
  for (const cat of categories) {
    ...
    items.push(
      `<button class="nav-cat-item" data-nav-category="${cat}">${label}</button>`,
    );
  }
...
function handleNavCategoryClick(e) {
  ...
  // Redirigimos al catálogo con el parámetro correcto
  window.location.href = `./catalogo.html?cat=${encodeURIComponent(category)}`;
}

// ─── Renderizado y Esqueletos ────────────────────────────────────
...
function injectSkeletonStyles() {
  ...
  s.textContent = `
    ...
    .skeleton {
      background: linear-gradient(90deg, var(--neutral-150) 25%, var(--neutral-100) 50%, var(--neutral-150) 75%);
  ...
  els.productDetail.innerHTML = `
    <div class="product-detail__gallery" data-product-gallery>
      <div class="product-detail__main-image-wrap">
        <img class="product-detail__main-image" src="${escapeHtml(images[0])}" alt="${escapeHtml(titulo)}" data-product-main-image loading="eager" />
        ${
          images.length > 1
            ? `
          <button class="product-detail__arrow product-detail__arrow--prev" type="button" aria-label="Imagen anterior" data-product-prev>
            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5"/></svg>
          </button>
          <button class="product-detail__arrow product-detail__arrow--next" type="button" aria-label="Imagen siguiente" data-product-next>
            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
          </button>
        `
            : ""
        }
        <div class="product-detail__zoom-hint" aria-hidden="true">
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607zM10.5 7.5v6m3-3h-6"/></svg> Zoom
        </div>
      </div>
      ...
      ${
        ...
        <div class="product-detail__thumbs">
          ${images
            .map(
              (img, i) => `
            <button class="product-detail__thumb ${i === 0 ? "is-active" : ""}" type="button" data-product-thumb data-index="${i}" aria-label="Ver imagen ${i + 1}">
    ...
    <div class="product-detail__info">
      ...
      <div class="product-detail__price">
        <span class="product-detail__price-main ${isConsult ? "price__value--muted" : ""}">${escapeHtml(price)}</span>
      </div>
      ${descripcionLarga ? `<div class="product-detail__description">${escapeHtml(descripcionLarga).replace(/\n/g, "<br>")}</div>` : ""}
      ...
      <div class="product-detail__actions">
        <button class="product-detail__add" type="button" data-add-to-cart="${escapeHtml(product.code)}" ${isConsult ? "disabled" : ""}>
          <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007z"/></svg>
          ${isConsult ? "Consultar precio" : "Agregar al carrito"}
        </button>
        <a class="product-detail__wa" href="${escapeHtml(`https://wa.me/5491131898284?text=${encodeURIComponent("Hola! Me interesa el producto: " + titulo + " (Cód: " + product.code + ")")}`)}">
...
function animateProductDetail() {
  ...
  const children = info.querySelectorAll(
    ".product-detail__title, .badges, .product-detail__price, .product-detail__description, .product-detail__meta-grid, .product-detail__trust, .product-detail__actions",
  );
...
function setupProductGallery() {
  ...
  mainWrap?.addEventListener("click", (e) => {
    if (
      e.target.closest("[data-product-prev]") ||
      e.target.closest("[data-product-next]")
    )
      return;
    ...
    if (!document.getElementById("zoom-styles")) {
      const s = document.createElement("style");
      s.id = "zoom-styles";
      s.textContent = `@keyframes fadeIn { from { opacity:0 } to { opacity:1 } } @keyframes scaleIn { from { transform:scale(0.92); opacity:0 } to { transform:scale(1); opacity:1 } }`;
      document.head.appendChild(s);
    }
...
function flyToCartAnim(sourceEl) {
  ...
  const img =
    sourceEl.closest(".card")?.querySelector(".card__img img") ||
...
function injectProductPageStyles() {
  ...
  s.textContent = `
    .nav__back-link { display: inline-flex; align-items: center; gap: var(--space-2); padding: 0 var(--space-4); height: var(--header-nav-h); color: rgba(255,255,255,0.8); font-size: var(--font-size-sm); font-weight: var(--font-weight-semibold); border-radius: var(--radius-sm); transition: color var(--duration-fast), background var(--duration-fast); white-space: nowrap; }
    .nav__back-link:hover { color: #fff; background: rgba(255,255,255,0.1); }
    .searchbar--disabled .searchbar__inner { opacity: 0.55; cursor: default; }
    .product-detail__zoom-hint { position: absolute; bottom: var(--space-3); right: var(--space-3); display: inline-flex; align-items: center; gap: 4px; font-size: var(--font-size-2xs); font-weight: var(--font-weight-medium); letter-spacing: var(--letter-spacing-wide); color: rgba(255,255,255,0.7); background: rgba(0,0,0,0.35); backdrop-filter: var(--blur-sm); padding: 4px 8px; border-radius: var(--radius-full); pointer-events: none; opacity: 0; transition: opacity var(--duration-base); }
    .product-detail__main-image-wrap:hover .product-detail__zoom-hint { opacity: 1; }
    .product-detail__trust { display: flex; flex-wrap: wrap; gap: var(--space-3); padding: var(--space-4) 0; border-top: 1px solid var(--color-border-subtle); }
    .trust-item { display: inline-flex; align-items: center; gap: var(--space-2); font-size: var(--font-size-xs); font-weight: var(--font-weight-medium); color: var(--color-text-secondary); padding: 5px 10px; border-radius: var(--radius-full); background: var(--color-surface-2); border: 1px solid var(--color-border-subtle); }
    .product-not-found { display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: var(--space-24) var(--space-8); grid-column: 1 / -1; gap: var(--space-4); }
    .product-not-found__icon { width: 72px; height: 72px; border-radius: var(--radius-full); background: var(--color-surface-2); display: grid; place-items: center; color: var(--color-text-muted); margin-bottom: var(--space-2); }
    .product-not-found__title { font-size: var(--font-size-2xl); font-weight: var(--font-weight-bold); color: var(--color-text); }
    .product-not-found__text { font-size: var(--font-size-base); color: var(--color-text-muted); max-width: 360px; }
    .product-not-found__btn { display: inline-flex; align-items: center; gap: var(--space-2); padding: 12px 28px; border-radius: var(--radius-full); background: var(--gradient-brand); color: #fff; font-size: var(--font-size-sm); font-weight: var(--font-weight-semibold); box-shadow: var(--shadow-brand); transition: box-shadow var(--duration-fast), transform var(--duration-fast) var(--ease-spring); }
    .product-not-found__btn:hover { box-shadow: var(--shadow-brand-lg); transform: translateY(-2px); }
  `;
  document.head.appendChild(s);
}
...
// ─── Inicialización (Boot) ───────────────────────────────────────
async function boot() {
  // 1. INICIAR LA INTERFAZ AL INSTANTE (¡Evita que el menú se congele!)
  initSharedUI();
  initProductSearch();
  ...
  try {
        ...
        els.productDetail.innerHTML = `
          <div class="product-not-found">
            <div class="product-not-found__icon">
              <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.4"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 15.75l-2.489-2.489m0 0a3.375 3.375 0 10-4.773-4.773 3.375 3.375 0 004.774 4.774zM21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
    ...
    document.addEventListener("fly-to-cart", (e) =>
      flyToCartAnim(e.detail.sourceEl),
    );
  } catch (err) {
